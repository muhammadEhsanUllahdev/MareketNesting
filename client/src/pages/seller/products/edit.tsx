...
export default function EditProduct() {
  ...
  const { data: product, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error(t("product.errors.fetchFailed"));
      return response.json();
    },
    enabled: !!id,
  });
  ...
  const handleProductUpdate = async (data: any) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || t("product.errors.updateFailed"));
      }
      ...
      toast({
        title: t("product.toast.successTitle"),
        description: t("product.toast.updateSuccess"),
      });
      ...
      console.error("Error updating product:", error);
      toast({
        title: t("product.toast.errorTitle"),
        description: error.message || t("product.toast.updateError"),
        variant: "destructive",
      });
    } finally {
      setIsUpdatingProduct(false);
        ...
        <div className="min-h-[400px] flex items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/dashboard/seller/products">
              <Button>
                <ArrowLeft className="h-4 w-4 mr-2" />
                {t("product.buttons.backToProducts")}
              </Button>
            </Link>
          </div>
        </div>
