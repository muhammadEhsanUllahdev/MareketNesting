...
//               Discover our selection of products with customer reviews,
//               questions and answers and detailed ratings.
//             </p>
//           </div>
...
export default function ProductsPage() {
  ...
  const { data: products = [], isLoading: productsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error(t("products.fetchError"));
      return response.json();
    },
  });
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error(t("categories.fetchError"));
      return response.json();
    },
  });
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="mb-6">
            ...
            <p className="text-gray-600">{t("products.headerDesc")}</p>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="w-48" data-testid="select-sort">
                  <SelectValue placeholder={t("products.sort.name")} />
                </SelectTrigger>
                <SelectContent>
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute top-2 left-2 flex flex-col gap-1">
                      {product.stock <= 5 && product.stock > 0 && (
                        <Badge className="bg-orange-500 text-white text-xs">
                          {product.stock <= 3
                            ? t("products.badge.fewLeft")
                            : t("products.badge.lowStock")}
                        </Badge>
                      )}
                    </div>
