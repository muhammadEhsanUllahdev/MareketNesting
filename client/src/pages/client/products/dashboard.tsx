...
export default function ClientProductsDashboard() {
  ...
  const { data: products = [], isLoading: productsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error(t("products.error.fetchFailed"));
      return response.json();
    },
  });
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/categories?language=${i18n.language}`,
        {
          credentials: "include",
        }
      );
      if (!response.ok)
        throw new Error(t("products.error.fetchCategoriesFailed"));
      return response.json();
    },
  });
  ...
  const filteredProducts = useMemo(() => {
    ...
    filtered.sort((a: Product, b: Product) => {
      switch (sortBy) {
        case "name":
          const nameA =
            a.translations?.[i18n.language]?.name ||
            a.translations?.en?.name ||
            t("products.unnamed");
          const nameB =
            b.translations?.[i18n.language]?.name ||
    ...
    if (stock === 0) {
      ...
      return (
        <Badge variant="secondary" className="bg-orange-100 text-orange-800">
          {t("products.stock.onlyLeft", { count: stock })}
        </Badge>
      );
    } else {
      return (
  ...
  return (
    <DashboardLayout
      ...
      subtitle={t("products.dashboard.subtitle")}
    >
      <div className="space-y-6">
        {/* Search and Filters */}
        <div className="bg-white p-6 rounded-lg border">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <SelectTrigger className="w-48" data-testid="select-category">
                    <SelectValue
                      placeholder={t("products.filter.allCategories")}
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="w-48" data-testid="select-sort">
                  <SelectValue placeholder={t("products.sort.nameAZ")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="name">
                    {t("products.sort.nameAZ")}
                  </SelectItem>
        ...
        <div className="bg-white p-6 rounded-lg border">
          {productsLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">{t("products.empty.subtitle")}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredProducts.map((product: Product) => {
                  ...
                  >
                    <div className="relative">
                      ...
                      <div className="absolute top-2 left-2 flex flex-col gap-1">
                        {product.stock <= 5 && product.stock > 0 && (
                          <Badge className="bg-orange-500 text-white text-xs">
                            {product.stock <= 3
                              ? t("products.badges.fewLeft")
                              : t("products.badges.lowStock")}
                          </Badge>
                        )}
                      </div>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="mb-2">
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          {t("products.serialNo")}: {product.sku}
                        </div>
                      </div>
                      ...
                      {/* Stock Status */}
                      <div className="mb-3">{getStockBadge(product.stock)}</div>
